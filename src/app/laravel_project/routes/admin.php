...
Route::prefix('admin')->middleware('auth:admins')->group(function () {
  ...
  Route::prefix('/user_watching')->group(function(){
    Route::get('/', [UserWatchingController::class, 'get'])->name('admin.user_watching'); //ユーザー管理画面（検索）
    Route::get('/inner', [UserInnerController::class, 'get']); //ユーザー詳細画面
  });
  Route::prefix('/community_search')->group(function(){
    Route::get('/', [CommunitySearchController::class, 'get'])->name('admin.community_search'); //コミュニティ管理画面（検索）
    Route::get('/inner',[CommunityInnerController::class, 'get']); //コミュニティ詳細画面
    Route::get('/chat',[CommunityChatController::class, 'get']); //チャット確認画面
  });
  Route::get('/admin_user', [AdminUserManagementController::class, 'get'])->name('admin.admin_user_management'); //管理者権限画面
  Route::get('/ng_word', [NgWordController::class, 'get'])->name('admin.ng_word'); //NGワード登録画面
  Route::prefix('/info')->group(function(){
    Route::get('/', [InfoController::class, 'get'])->name('admin.info');//お問わせ画面
